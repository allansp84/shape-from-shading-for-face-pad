...
class CasiaRaUVAD(Dataset):
    def __init__(self, dataset_path, output_path='./working', face_locations_path=None, file_types=('.avi', '.mov',),
        ...
        super(CasiaRaUVAD, self).__init__(dataset_path, output_path, face_locations_path, file_types, operation, max_axis,
        ...
        self.uvad = UVAD(os.path.join(dataset_path, 'uvad'),
                         output_path=output_path,
                         face_locations_path=os.path.join(dataset_path, 'uvad', 'face-locations-v3'),
                         file_types=file_types,
                         operation=operation,
                         max_axis=max_axis,
                         n_channel=n_channel,
    ...
    def build_meta(self, inpath, filetypes, frame_numbers=1):
        ...
        in_path_uvad = inpath.replace(self.dataset_path.split("/")[-1], os.path.join(self.dataset_path.split("/")[-1], 'uvad'))
        r_dict_uvad = self.uvad.build_meta(in_path_uvad, filetypes, frame_numbers)
        ...
        shift_idx = r_dict_uvad['all_idxs'].max() + 1

        all_fnames = np.concatenate((all_fnames, r_dict_uvad['all_fnames']))
        all_labels = np.concatenate((all_labels, r_dict_uvad['all_labels']))
        all_idxs = np.concatenate((all_idxs, r_dict_uvad['all_idxs'] + shift_idx))

        train_idxs = np.concatenate((train_idxs, r_dict_uvad['train_idxs'] + shift_idx))
        devel_idxs = np.concatenate((devel_idxs, r_dict_uvad['devel_idxs'] + shift_idx))
        test_all_idxs = np.concatenate((test_all_idxs, r_dict_uvad['test_all_idxs'] + shift_idx))

        train_grand = np.concatenate((train_grand,
                                      r_dict_uvad['train_scenarios_idxs']['train_grand'] + shift_idx))

        devel_grand = np.concatenate((devel_grand,
                                      r_dict_uvad['devel_scenarios_idxs']['devel_grand'] + shift_idx))

        test_grand = np.concatenate((test_grand,
                                     r_dict_uvad['test_scenarios_idxs']['test_grand'] + shift_idx))
